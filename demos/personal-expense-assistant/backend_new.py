...
async def chat(request: ChatRequest = Body(...), app_context: AppContexts = Depends(get_app_contexts)) -> ChatResponse:
    ...
    if not app_context.session_service.get_session(app_name=AGENT_APP_NAME, user_id=user_id, session_id=session_id):
    ...
    try:
        ...
        return ChatResponse(response=final_response_text, attachments=base64_attachments)
